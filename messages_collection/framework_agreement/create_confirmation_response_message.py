""" Prepare expected message for platform, the Create Confirmation Response process of Framework Agreement procedure."""
import copy
import fnmatch
...
class CreateConfirmationResponseMessage:
    ...
    def build_expected_platform_message(self, actual_message, expected_quantity_of_outcomes_objects):
        """Build the message."""
        ...
        outcomes_confirmationresponses_array = list()
        for obj in range(expected_quantity_of_outcomes_objects):
            outcomes_confirmationresponses_array.append(copy.deepcopy(
                self.__platform_message['data']['outcomes']['confirmationResponses'][0]))

            is_requests_id_correct = is_it_uuid(actual_message["data"]["outcomes"]["confirmationResponses"][obj]["id"])

            if is_requests_id_correct is True:
                outcomes_confirmationresponses_array[obj]['id'] = \
                    actual_message["data"]["outcomes"]["confirmationResponses"][obj]["id"]
            else:
                ValueError("The message is not correct: 'data.outcomes.confirmationResponses.id'.")

        self.__platform_message['data']['outcomes']['confirmationResponses'] = outcomes_confirmationresponses_array
