...
@pytest.fixture(scope="function")
# Create EI: full data model, create FS: full data model, де payer/funder/buyer - це одна організація.
def create_fs_tc_2_new(get_parameters, connect_to_keyspace, create_ei_tc_1):
    bpe_host = get_parameters[2]
    country = get_parameters[4]
    clean_up_database = get_parameters[10]
...
def create_fs_tc_3_new(get_parameters, connect_to_keyspace, create_ei_tc_2):
    bpe_host = get_parameters[2]
    country = get_parameters[4]
    clean_up_database = get_parameters[10]
