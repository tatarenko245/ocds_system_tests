"""Prepare the expected releases of the qualification process, framework agreement procedures."""
import copy

from functions_collection.prepare_date import get_min_date


class QualificationRelease:
    """This class creates instance of release."""

    def __init__(self, environment,  payload, actual_message, ocid):
    ...
    def build_expected_fe_release(self, previous_fe_release, actual_fe_release, qualification_id,
        ...
        """Prepare 'qualifications' array for expected FE release: releases[0].qualification: FR.COM-7.20.1,
        FR.COM-7.20.9,  FR.COM-7.22.1, FR.COM-7.22.3"""
        # Set a specific statusDetails and enrich the specified qualification object by id.
        expected_qualifications_array = None
        if "qualifications" in actual_fe_release['releases'][0]:
            expected_qualifications_array = copy.deepcopy(actual_fe_release['releases'][0]['qualifications'])
        else:
            KeyError("'qualifications' key was missed into Actual FE release.")

        for q in range(len(expected_qualifications_array)):
            if expected_qualifications_array[q]['id'] == qualification_id:

                expected_qualifications_array[q]['statusDetails'] = self.__payload['qualification']['statusDetails']
                expected_qualifications_array[q]['date'] = self.__actual_message['data']['operationDate']
                if "internalId" in self.__payload['qualification']:
                    expected_qualifications_array[q]['internalId'] = self.__payload['qualification']['internalId']

                if "description" in self.__payload['qualification']:
                    expected_qualifications_array[q]['description'] = self.__payload['qualification']['description']

                if "documents" in self.__payload['qualification']:
                    del expected_qualifications_array[q]['documents'][0]
                    new_qualification_documents_array = list()
                    for d in range(len(self.__payload['qualification']['documents'])):
                        new_qualification_documents_array.append(copy.deepcopy(
                            self.__expected_fe_release['releases'][0]['qualifications'][0]['documents'][0]
                        ))
                        new_qualification_documents_array[d]['id'] = \
                            self.__payload['qualification']['documents'][d]['id']
                        new_qualification_documents_array[d]['documentType'] = \
                            self.__payload['qualification']['documents'][d]['documentType']
                        new_qualification_documents_array[d]['title'] = \
                            self.__payload['qualification']['documents'][d]['title']
                        if "description" in self.__payload['qualification']['documents'][d]:
                            new_qualification_documents_array[d]['description'] = \
                                self.__payload['qualification']['documents'][d]['description']
                        else:
                            del new_qualification_documents_array[d]['description']
                        new_qualification_documents_array[d]['url'] = \
                            f"{self.__metadata_document_url}/{new_qualification_documents_array[d]['id']}"
                        new_qualification_documents_array[d]['datePublished'] = \
                            self.__actual_message['data']['operationDate']
                    expected_qualifications_array[q]['documents'] = new_qualification_documents_array

        # Set a specific statusDetails for another qualification objects by id.
        if actual_fe_release['releases'][0]['tender']['otherCriteria']['reductionCriteria'] == \
                "scoring" and actual_fe_release['releases'][0]['tender']['otherCriteria'][
                'qualificationSystemMethods'][0] == "automated":
            all_qualifications_id_list = list()
            for q in range(len(expected_qualifications_array)):
