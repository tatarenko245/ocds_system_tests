...
from data_collection.data_constant import locality_scheme_tuple, region_id_tuple, person_title_tuple, \
    business_function_type_1_tuple, type_of_supplier_tuple, scale_tuple, \
    documentType_for_create_submission_framework_agreement
from functions_collection.prepare_date import old_period
from functions_collection.some_functions import get_locality_id_according_with_region_id, \
...
class CreateSubmissionPayload:
    ...
    def delete_optional_fields(
        ...
        """Call this method before method 'prepare_submission_object' ! Delete optional fields from payload."""
    ...
    def prepare_submission_object(
                self, submission_position, quantity_of_candidates=1, quantity_of_additional_identifiers=1,
            quantity_of_persones=1, quantity_of_evidences=1, quantity_of_business_functions=1,
            quantity_of_bf_documents=1, quantity_of_main_economic_activities=1, quantity_of_bank_accounts=1,
        ...
        for c_0 in range(quantity_of_candidates):
            ...
            candidates_array[c_0]['name'] = f"create submission[{submission_position}]: " \
                                            f"submission.candidates[{c_0}].name"
            candidates_array[c_0]['identifier']['id'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].identifier.id"

            candidates_array[c_0]['identifier']['legalName'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].identifier.legalName"
            ...
            if "uri" in candidates_array[c_0]['identifier']:
                candidates_array[c_0]['identifier']['uri'] = \
                    f"create submission[{submission_position}]: submission.candidates[{c_0}].identifier.uri"
            ...
            if "additionalIdentifiers" in candidates_array[c_0]:
                ...
                for q_0 in range(quantity_of_additional_identifiers):
                    ...
                    additional_identifier_array[q_0]['id'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}]." \
                        f"additionalIdentifiers[{q_0}]['id']"

                    additional_identifier_array[q_0]['legalName'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}]." \
                        f"additionalIdentifiers[{q_0}]['legalName']"

                    additional_identifier_array[q_0]['scheme'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}]." \
                        f"additionalIdentifiers[{q_0}]['scheme']"

                    if "uri" in additional_identifier_array[q_0]:
                        additional_identifier_array[q_0]['uri'] = \
                            f"create submission[{submission_position}]: submission.candidates[{c_0}]." \
                            f"additionalIdentifiers[{q_0}]['uri']"

                candidates_array[c_0]['additionalIdentifiers'] = additional_identifier_array

            candidates_array[c_0]['address']['streetAddress'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].address.streetAddress"

            if "postalCode" in candidates_array[c_0]['address']:
                candidates_array[c_0]['address']['postalCode'] = \
                    f"create submission[{submission_position}]: submission.candidates[{c_0}].address.postalCode"
            ...
            candidates_array[c_0]['address']['addressDetails']['country']['description'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].address." \
            ...
            candidates_array[c_0]['address']['addressDetails']['region']['description'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}]." \
                f"address.addressDetails.region.description"
            ...
            candidates_array[c_0]['address']['addressDetails']['locality']['description'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}]." \
                f"address.addressDetails.locality." \
                f"description"

            candidates_array[c_0]['contactPoint']['name'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].contactPoint.name"

            candidates_array[c_0]['contactPoint']['email'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].contactPoint.email"

            candidates_array[c_0]['contactPoint']['telephone'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].contactPoint.telephone"

            if "faxNumber" in candidates_array[c_0]['contactPoint']:
                candidates_array[c_0]['contactPoint']['faxNumber'] = \
                    f"create submission[{submission_position}]: submission.candidates[{c_0}].contactPoint.faxNumber"

            if "url" in candidates_array[c_0]['contactPoint']:
                candidates_array[c_0]['contactPoint']['url'] = \
                    f"create submission[{submission_position}]: submission.candidates[{c_0}].contactPoint.url"
            ...
            if "persones" in candidates_array[c_0]:
                ...
                for p_0 in range(quantity_of_persones):
                    ...
                    persones_array[p_0]['title'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}].persones[{p_0}].title"

                    persones_array[p_0]['title'] = f"{random.choice(person_title_tuple)}"

                    persones_array[p_0]['name'] = f"create submission[{submission_position}]: " \
                                                  f"submission.candidates[{c_0}].perones[{p_0}].name"

                    persones_array[p_0]['identifier']['scheme'] = \
                        f"create submission[{submission_position}]: " \
                        f"submission.candidates[{c_0}].perones[{p_0}].identifier.scheme"

                    persones_array[p_0]['identifier']['id'] = \
                        f"create submission[{submission_position}]: " \
                        f"submission.candidates[{c_0}].perones[{p_0}].identifier.id"

                    if "uri" in persones_array[p_0]['identifier']:
                        persones_array[p_0]['identifier']['uri'] = \
                            f"create submission[{submission_position}]: " \
                            f"submission.candidates[{c_0}].perones[{p_0}].identifier.uri"
                    ...
                    for bf_0 in range(quantity_of_business_functions):
                        ...
                        business_functions_array[bf_0]['jobTitle'] = \
                            f"create submission[{submission_position}]: submission.candidates[{c_0}].perones[{p_0}]." \
                            f"businessFunctions[{bf_0}].jobTitle"

                        business_functions_array[bf_0]['period']['startDate'] = old_period()[0]
                        ...
                        if "documents" in business_functions_array[bf_0]:
                            ...
                            for bf_1 in range(quantity_of_bf_documents):
                                ...
                                bf_documents_array[bf_1]['title'] = \
                                    f"create submission[{submission_position}]n: " \
                                    f"submission.candidates[{c_0}].perones[{p_0}]." \
            ...
            if "mainEconomicActivities" in candidates_array[c_0]['details']:
                ...
                for d_0 in range(quantity_of_main_economic_activities):
                    ...
                    main_economic_activities_array[d_0]['description'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}].details." \
            ...
            if "bankAccounts" in candidates_array[c_0]['details']:
                ...
                for d_1 in range(quantity_of_bank_accounts):
                    ...
                    bank_accounts_array[d_1]['address']['addressDetails']['country']['description'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}].details." \
                        f"bankAccounts[{d_1}].address.addressDetails.country.description"
                    ...
                    bank_accounts_array[d_1]['address']['addressDetails']['region']['description'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}].details." \
                        f"bankAccounts[{d_1}].address.addressDetails.region.description"
                    ...
                    bank_accounts_array[d_1]['address']['addressDetails']['locality']['description'] = \
                        f"create submission[{submission_position}]: submission.candidates[{c_0}].details." \
                        f"bankAccounts[{d_1}].address.addressDetails.locality.description"
                    ...
                    del bank_accounts_array[d_1]['additionalAccountIdentifiers'][0]
                    if "additionalAccountIdentifiers" in bank_accounts_array[d_1]:
                        ...
                        for d_2 in range(quantity_of_additional_account_identifiers):
                            ...
                            additional_account_identifiers_array[d_2]['scheme'] = "fiscal"
                            additional_account_identifiers_array[d_2]['id'] = f"{c_0 + d_1 + d_2}"
                        bank_accounts_array[d_1]['additionalAccountIdentifiers'] = additional_account_identifiers_array
            ...
            candidates_array[c_0]['details']['legalForm']['description'] = \
                f"create submission[{submission_position}]: submission.candidates[{c_0}].details.legalForm.description"

            if "uri" in candidates_array[c_0]['details']['legalForm']:
                candidates_array[c_0]['details']['legalForm']['uri'] = \
                    f"create submission[{submission_position}]: submission.candidates[{c_0}].details.legalForm.uri"
                        ...
                        if "evidences" in requirement_responses_object:
                            ...
                            for q_2 in range(quantity_of_evidences):
                                ...
                                evidences_array[q_2]['title'] = \
                                    f"create submission[{submission_position}]: " \
                                    f"submission.requirementResponses[{q_0}]." \
                                    f"evidences[{q_2}].title"

                                evidences_array[q_2]['description'] = \
                                    f"create submission[{submission_position}]: " \
                                    f"submission.requirementResponses[{q_0}].evidences[{q_2}].description"
        ...
        if "documents" in self.__payload['submission']:
            ...
            for sd_0 in range(quantity_of_documents):
                ...
                documents_array[sd_0]['title'] = f"create submission[{submission_position}]: " \
                                                 f"submission.documents[{sd_0}].title"
                if "description" in documents_array[sd_0]:

                    documents_array[sd_0]['description'] = \
                        f"create submission[{submission_position}]: submission.documents[{sd_0}].description"
